<div class="profile-content">
    <div class="profile-header">
        <div class="profile-main-img">
            <a href="<?php echo adforest_set_url_param(get_author_posts_url($author->ID), 'type', 'ads'); ?>">
                <img src="<?php echo esc_attr($user_pic); ?>" id="user_dp"
                    alt="<?php echo __('Profile Picture', 'adforest'); ?>" class="img-fluid">
            </a>
            <?php
        ...
        if (get_user_meta($author->ID, '_sb_badge_text', true) != "" && isset($adforest_theme['sb_enable_user_badge']) && $adforest_theme['sb_enable_user_badge']) {
            ?>
            <div class="profile-dealer verify-bage"><span
                    class="pro-dealer2 label<?php echo get_user_meta($author->ID, '_sb_badge_type', true); ?>"><?php echo adforest_returnEcho(get_user_meta($author->ID, '_sb_badge_text', true)); ?></span>
            </div>
            <?php
        }
        ?>
    ...
    <div class="profile-heading">
        <h4><a
                href="<?php echo adforest_set_url_param(get_author_posts_url($author->ID), 'type', 'ads'); ?>"><?php echo esc_html($author->display_name); ?></a>
        </h4>
        <?php
        ...
        <ul class="add-phone-list">
            ...
            <?php if ($contact_num != "") {
                if (adforest_showPhone_to_users()) {
                    ...
                    ?>
                    <li> <a href="<?php echo esc_url($redirect_url); ?>" class="sb-click-num-user2 phone-list" id="show_ph_div"
                            data-user_id="<?php echo esc_attr($author->ID); ?>">
                            <span class="info-heading"><i class="fa fa-phone"></i>
                                <?php echo esc_html__('Phone:', 'adforest') ?> </span>
            ...
            <li><a href="javascript:void(0)" class="address-list"><span><i class="fa fa-location-arrow"></i>
                        <?php echo esc_html__('Address:', 'adforest'); ?> </span>
                    <?php echo get_user_meta($author->ID, '_sb_address', true); ?> </a></li>
            <li><a href="javascript:void(0)" class="hour-list"><span><i
                            class="fa fa-calendar"></i><?php echo esc_html__('Last Active:', 'adforest') ?></span>
                    <?php printf(_x('%s Ago', 'Last login time', 'adforest'), adforest_get_last_login($author->ID)); ?></a>
            </li>
        </ul>
    </div>
