    ...
    function ad_post_short_base_func($atts, $content = '')
    {
        ...
        if (isset($_GET['id'])) {
            ...
            if (get_post_field('post_author', $id) != get_current_user_id() && !is_super_admin(get_current_user_id())) {
                ...
                if ($level >= 5) {
                    $ad_sub_sub_sub_sub_cats = adforest_get_cats('ad_cats', $cats[3]['id'], 0, 'post_ad');
                    $sub_sub_sub_sub_cats_html = '';
                    foreach ($ad_sub_sub_sub_sub_cats as $ad_cat) {
                        $selected = '';
                        if ($level > 0 && $ad_cat->term_id == $cats[4]['id']) {
                            $selected = ' selected="selected"';
                            $selected_cats_list .= '<li> ' . $ad_cat->name . ' </li> ';
                        }
                        $sub_sub_sub_sub_cats_html .= '<option value="' . $ad_cat->term_id . '" ' . $selected . ' data-name = "' . $ad_cat->name . '" >' . $ad_cat->name . '</option>';
                    }
                }
...
//                    Monkey va plant bug myth
		...
		<div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
            ...
            if ($mapType == 'leafletjs_map') {
                $lat_lon_script = '<script type="text/javascript">var mymap = L.map(\'dvMap\').setView([' . $pin_lat . ', ' . $pin_long . '], 13);
                 ...
                 var searchControl 	=	new L.Control.Search({url: \'//nominatim.openstreetmap.org/search?format=json&q={s}\',
                 jsonpParam: \'json_callback\',
                 propertyName: \'display_name\',
                 propertyLoc: [\'lat\',\'lon\'],
                 marker: markerz,autoCollapse: true,autoType: true,minLength: 2,});
                 searchControl.on(\'search:locationfound\', function(obj) {		var lt	=	obj.latlng + \'\';
                    var res = lt.split( "LatLng(" );
                    res = res[1].split( ")" );
                    res = res[0].split( "," );
                    document.getElementById(\'ad_map_lat\').value = res[0];document.getElementById(\'ad_map_long\').value = res[1];});
                    mymap.addControl( searchControl );markerz.on(\'dragend\', function (e) {document.getElementById(\'ad_map_lat\').value = markerz.getLatLng().lat;
                        ...
                        new Autocomplete("sb_user_address", {
                            ...
                            onSubmit: ({ object }) => {
                                ...
                                if (markerz) {
                                    mymap.removeLayer(markerz);
                                }
                                markerz = L.marker([lat, lng], { draggable: true }).addTo(mymap);

                                // Update form fields
                                document.getElementById("ad_map_lat").value = lat;
                                document.getElementById("ad_map_long").value = lng;
                        
                                // Dragend event listener for marker
            ...
            $lat_long_html = $for_g_map . '<div class="col-md-6 col-lg-6 col-xs-12 col-sm-12">
            ...
            if (isset($adforest_theme['allow_featured_on_ad']) && $adforest_theme['allow_featured_on_ad']) {
				...
				' . __('Packages. ', 'adforest') . '
                    ...
                    if ($total_featured_ads > 0) {
                        ...
                        if ($total_featured_ads > 0) {
                            $count_featured_ads = __('Featured ads remaining:', 'adforest') . $total_featured_ads;
                        }
                        $feature_text = '';
                        if (isset($adforest_theme['sb_feature_desc']) && $adforest_theme['sb_feature_desc'] != "") {
                            $feature_text = $adforest_theme['sb_feature_desc'];
        ...
        $categories_style_html = '<div class="form-group cats-dropdown"> <div class="row">
                                   ...
                                   <div class="row" id="ad_cat_sub_sub_sub_sub_div">
                                        <label class="control-label"><small>' . __('Select suitable subcategory for your ad', 'adforest') . '</small></label>
                                           <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
                                ...
                                <div class="sub-item ad-user-information">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            ...
                                            <div class="term-condition adforest-ad-post-terms">
                                                <div class="row">
                                                    ' . $tems_cond_field . ' 
