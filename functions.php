...
function adforest_scripts()
{
    ...
    if ($mapType == 'leafletjs_map') {
        ...
        //  Leaf Let Full Screen 
        wp_register_script('leaflet-fullscreen', trailingslashit(get_template_directory_uri()) . 'assests/leaflet/leaflet-fullscreen.js', false, false, false);

        wp_enqueue_style('leaflet-full', trailingslashit(get_template_directory_uri()) . 'assests/leaflet/leaflet-fullscreen.css');

        wp_enqueue_style(
            'autocomplete-css',
            'https://cdn.jsdelivr.net/gh/tomickigrzegorz/autocomplete@1.8.3/dist/css/autocomplete.min.css'
        );
        wp_register_script(
            'autocomplete-js',
            'https://cdn.jsdelivr.net/gh/tomickigrzegorz/autocomplete@1.8.3/dist/js/autocomplete.min.js',
            false,
            false,
            false
        );
...
function sb_validate_code_daily_fun()
{
    ...
    if ($pc && $kn) {
        $inf = array(
            'method' => 'POST', 'timeout' => 45, 'redirection' => 5, 'httpversion' => '1.0', 'blocking' => true,
...
$pro_badge_image = isset($adforestAPI['pro_badge_image']['url']) ? true : false;
